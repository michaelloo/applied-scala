...
object IOExercises {
  ...
  def showCurrentTempInF(currentTemp: IO[Celsius], converter: Celsius => IO[Fahrenheit]): IO[String] = {
    currentTemp.flatMap(converter).attempt.map(result =>
      result match {
        case Left(error) => error.getMessage
        case Right(temperature) => "The temperature is " + temperature.value
      }
    )
  }
...
//      .map(_ => logger("executing step 2"))
//      .map(_ => logger("executing step 3"))
