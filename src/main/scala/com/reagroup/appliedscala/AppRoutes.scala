...
class AppRoutes(fetchAllMoviesHandler: IO[Response[IO]],
  ...
  val openRoutes: HttpRoutes[IO] = HttpRoutes.of {
    ...
    case GET -> Root / "movies" / LongVar(id) :? OptionalEnrichedMatcher(optEnriched) => if (optEnriched.contains(true)) fetchEnrichedMovieHandler(id) else fetchMovieHandler(id)
